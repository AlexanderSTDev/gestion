...
class CompartidosModel extends Query
{
    ...
    {
        $sql = "SELECT d.id, d.fecha_add, d.correo, a.nombre, a.tipo, a.id_carpeta, u.correo AS compartido, u.nombre AS usuario FROM detalle_archivos d INNER JOIN archivos a ON d.id_archivo = a.id INNER JOIN carpetas c ON a.id_carpeta = c.id INNER JOIN usuarios u ON d.id_usuario = u.id WHERE d.id = $id_detalle ORDER BY d.id DESC";
